...
def t1Insert(meta):
    t = meta.tables['postgres_webhook_test_t1']
    return {
        "name": "t1: insert",
        "statement": t.insert().values(c1=1, c2='hello'),
        "resp": {
            'table': 'postgres_webhook_test_t1',
            'schema': 'public',
            'op': 'INSERT',
            'data': {'c1': 1, 'c2': 'hello'}
        }
...
def t3Insert(meta):
    t = meta.tables['postgres_webhook_test_t3']
    return {
        "name": "t3: insert",
        "statement": t.insert().values(c1=1, c2='hello'),
        "resp": {
            'table': 'postgres_webhook_test_t3',
            'schema': 'public',
            'op': 'INSERT',
            'data': {'c1': 1}
        }
...
def t4Insert(meta):
    t = meta.tables['postgres_webhook_test_t4']
    return {
        "name": "t4: insert",
        "statement": t.insert().values(c1=1, c2='hello', c3='world'),
        "resp": {
            'table': 'postgres_webhook_test_t4',
            'schema': 'public',
            'op': 'INSERT',
...
engine = create_engine('postgresql://admin@localhost:5432/postgres_webhook_test')
meta = MetaData()
